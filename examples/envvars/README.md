...
AGENTBOX_DEBUG=1 \
AGENTBOX_FORCE=1 \
AGENTBOX_HOSTNAME="TANAABAGENTBOXENV${GITHUB_RUN_ID:-LOCAL}" \
...
test "$(scutil --get ComputerName)" = "TANAABAGENTBOXENV${GITHUB_RUN_ID:-LOCAL}"
test "$(scutil --get HostName)" = "TANAABAGENTBOXENV${GITHUB_RUN_ID:-LOCAL}"
test "$(scutil --get LocalHostName)" = "TANAABAGENTBOXENV${GITHUB_RUN_ID:-LOCAL}"

# should derive the Tailscale hostname from the TANAAB-prefixed canonical hostname
tailscale status --json | jq -e --arg host "AGENTBOXENV${GITHUB_RUN_ID:-LOCAL}" '.Self.HostName == $host'
