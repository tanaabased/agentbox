...
export const AGENTBOX_CONFIG_SCHEMA_VERSION = 2;
...
async function pngFileExists(path) {
  if (!(await regularFileExists(path))) return false;
...
async function payloadRootForScript(scriptPath) {
  ...
  for (const candidate of candidates) {
    const requiredResults = await Promise.all(
      requiredPayloadPaths.map((relativePath) => regularFileExists(join(candidate, relativePath))),
    );
    if (!requiredResults.every(Boolean)) continue;
    const assets = await readdir(join(candidate, 'assets'), { withFileTypes: true });
    const profileResults = await Promise.all(
      assets
        .filter((entry) => entry.isFile() && /^profile.*[.]png$/.test(entry.name))
        .map((entry) => pngFileExists(join(candidate, 'assets', entry.name))),
    );
    if (
      (await pngFileExists(join(candidate, 'assets', 'default_avatar.png'))) &&
      profileResults.some(Boolean)
    ) {
      return candidate;
    }
  }
...
export function validateAgentboxInstallationConfig(config) {
  ...
  if (typeof config.linkCommand !== 'boolean') {
    throw configError('agentbox installation config linkCommand must be a boolean.');
  }
  if (
    !config.installations ||
    typeof config.installations !== 'object' ||
    Array.isArray(config.installations)
  ) {
...
export function withAgentboxInstallation(config, key, installation, options = {}) {
  ...
  const nextConfig = {
    ...
    linkCommand: options.linkCommand ?? config.linkCommand,
    installations: {
      ...config.installations,
      [key]: { ...installation },
    },
...
export function withDefaultAgentboxInstallation(config, key, options = {}) {
