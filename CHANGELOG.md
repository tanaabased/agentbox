...
- Fixed OpenClaw setup and reruns to stop overwriting `ui.assistant.name` and `ui.assistant.avatar`; `ui.seamColor` remains managed. [#26](https://github.com/tanaabased/agentbox/pull/26)
- Fixed Tailscale operator and status checks in root-published health reports when launchd uses a minimal `PATH`. [#23](https://github.com/tanaabased/agentbox/issues/23)
